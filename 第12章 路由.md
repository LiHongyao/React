...
[React Router](https://reactrouter.com/) 是一个基于 [React](https://zh-hans.reactjs.org/) 之上的强大路由库，它可以让你向应用中快速地添加视图和数据流，同时保持页面与 URL 间的同步。

本小节主要参照教程讲解 React-Router 的基本使用，更多细节参照官网示例。
...
- [< BrowserRouter >](https://reactrouter.com/en/main/router-components/browser-router)：`/` 斜杠形式显示路由路径 => *`http://localhost:3000/search`*
- [< HashRouter >](https://reactrouter.com/en/main/router-components/hash-router)：`#` 哈希形式显示路由路径 => *`http://localhost:3000/#/search`*
...
```shell
$ npm create vite@latest react-routers  -- --template react-ts
$ cd react-routers && npm install
