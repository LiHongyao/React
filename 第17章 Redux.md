...
Redux 是JS 应用的状态容器，提供可预测的状态管理（数据流管理）。
...
Redux 的出现改变了react的这种窘迫处境，它提供了整个应用的唯一数据源store，这个数据源是随处可以访问的，不需要靠父子相传，并且还提供了（间接）更新这个数据源的方法，并且是随处可使用的！
...
**store** 是Redux提供的唯一数据源，它存储了整个应用的 state。Store 有以下职责：
...
这样做将使 action 创建函数更容易被移植和测试。当调用 action 创建函数时，一般会把 action 创建函数的结果传给 `dispatch()` 方法即可发起一次 dispatch 过程。类似于Vuex中的 Commit 提交一次 Mutaions.

```js
store.dishpatch(CHANGE_NAME("李好帅"));
```

## 3. Reducer

只是一个接收 state 和action，并返回新的 state 的函数。
...
3）创建reducers：*`store/reducers.js`*
...
```
初始化状态： { card: { name: 'Li-HONGYAO', picture: 'xxx.jpg' }, dialog: { status: false }}
数据已更新： { card: { name: '李好帅', picture: 'xxx.jpg' }, dialog: { status: false }}
数据已更新： { card: { name: '李好帅', picture: 'xxx.png' }, dialog: { status: false }}
数据已更新： { card: { name: '李好帅', picture: 'xxx.png' }, dialog: { status: true }}
数据已更新： { card: { name: '李好帅', picture: 'xxx.png' }, dialog: { status: false }}
```
